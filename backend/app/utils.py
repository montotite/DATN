...
class Crud:
    ...
    def update_telemetry(self, entity_id: str, key: str, ts: int, value: str):
        try:
            ...
            data = data.filter(Telemetry.key == str(key),
    ...
    def create_telemetry(self, entity_id: str, key: str, value: str):
        try:
            data = Telemetry(entity_id=str(entity_id),
                             key=str(key),
                             value=str(value))
    ...
    def get_oder_info(self, device_id: str, ts: str):
        data = self.db.query(Telemetry.entity_id,
                             Telemetry.ts,
    ...
    def get_oder_list(self,
        ...
        if propositions:
            data = data.filter(Telemetry.value.cast(pg.JSON)[
                               'proposition'].astext.cast(sa.String).in_(tuple(propositions)))
        if shipping:
            data = data.filter(Telemetry.value.cast(pg.JSON)[
                'shipping'].astext.cast(sa.String).in_(tuple(shipping)))
        if username:
            data = data.filter(Telemetry.value.cast(pg.JSON)[
                'username'].astext.cast(sa.String).in_(tuple(username)))
        if phone:
            data = data.filter(Telemetry.value.cast(pg.JSON)[
                'phone'].astext.cast(sa.String).in_(tuple(phone)))
        total = data.count()
        ...
        return [{'entity_id': item.entity_id,
                 'ts': item.ts,
                 'value': item.value}for item in data], total

    def delete_relation_by_from_id(self,
                                   from_id: str = None,
                                   from_type: str = None):
        try:
            data = self.db.query(Relation)
            data = data.filter(Relation.from_id == str(from_id),
    ...
    def delete_relation_by_to_id(self,
                                 to_id: str = None,
                                 to_type: str = None):
        try:
            data = self.db.query(Relation)
            data = data.filter(Relation.to_id == str(to_id),
    ...
    def find_relation(self,
        ...
        if relation_type_group:
            data = data.filter(Relation.relation_type_group ==
                               str(relation_type_group))

        data = data.all()
        return [{'from_id': item.from_id,
                 'from_type': item.from_type,
                 'to_id': item.to_id,
                 'to_type': item.to_type,
                 'relation_type': item.relation_type,
                 'relation_type_group': item.relation_type_group,
                 'additional_info': item.additional_info} for item in data]

    def delete_relation(self, from_id: str, from_type: str, to_id: str, to_type: str, relation_type: str, relation_type_group: str):
        try:
            data = self.db.query(Relation)
            data = data.filter(Relation.from_id == str(from_id),
                               Relation.from_type == str(from_type),
    ...
    def create_relation(self, from_id: str, from_type: str, to_id: str, to_type: str, relation_type: str, relation_type_group: str):
        try:
            data = Relation(from_id=str(from_id),
    ...
    def get_atribute_value(self, id: str, scope: str, keys: list):
        ...
        data = data.filter(Attribute.entity_id == str(id),
        ...
        data = data.all()
        data = [{'attribute_key': item.attribute_key,
                 'value': item.value,
                'ts': item.last_update_ts} for item in data]
        return data
    ...
    def get_asset_list(self, offset=None, limit=None):
        ...
        return [{'id': str(item.id),
                'created_time': item.created_time,
                 ...
                 'type': item.type,
                 'name': item.name} for item in data], total
    ...
    def get_device_list(self, offset=None, limit=None):
        ...
        return [{'id': str(item.id),
                'created_time': item.created_time,
                 ...
                 'type': item.type,
                 'name': item.name,
                 'credential': item.credential} for item in data], total
